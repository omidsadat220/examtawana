...
use App\Mail\GoogleAccountCreatedMail;
...
Route::get('/auth/google/callback', function () {
    try {
        ...
        $user = User::where('email', $googleUser->email)
                   ...
                   // Inside your Google callback
            if (!$user) {
                // 1️⃣ Generate plain password first
                ...
                Mail::to($user->email)->send(
                    new GoogleAccountCreatedMail($plainPassword)
                );
            } else {
            // Update existing user
            $user->update([
                'google_id' => $googleUser->id,
                'avatar' => $googleUser->avatar, // Update profile picture
            ]);
        }
